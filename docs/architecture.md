...
The OTP password is the entire trust boundary for writes. Anyone who knows the password (or a token minted from it) can POST to `content.mjs` and write content into the `site-content` blob. That content is then injected as `el.innerHTML` on every page load (`hidden-gem/js/editor.js:191`) and as a BeautifulSoup-parsed fragment by the bake (`scripts/bake_hidden_gem_edits.py:225`). Both render paths first run the value through an **allowlist sanitizer**: allowed tags are `a, b, br, blockquote, em, i, li, ol, p, span, strong, u, ul`; allowed attributes are `href` on `<a>` only, with URL-scheme vetting that rejects `javascript:`, `data:`, `vbscript:`. Everything else gets unwrapped (text survives, markup discarded); `<script>`, `<style>`, `<iframe>`, `<svg>`, `<form>`, and media tags get decomposed (content discarded). The two sanitizers — `sanitizeOverrideHTML()` at `hidden-gem/js/editor.js:143` and `sanitize_override_html()` at `scripts/bake_hidden_gem_edits.py:64` — must produce identical output for identical input. Widening the password set, or weakening it via the `'chloe'` default (the `FALLBACK_PASSWORD` in `otp.mjs:21`), widens the content-overwrite surface (an attacker can replace legitimate copy with their own) but no longer widens the XSS surface. Server-side write sanitization in `content.mjs` is not implemented; the storage layer accepts what the client POSTs and relies on the two-sided render sanitizer to keep visitors safe.
...
- The bake sanitizes through an allowlist of safe tags + attributes (`sanitize_override_html()` at `scripts/bake_hidden_gem_edits.py:64`, mirrored in `editor.js:143`). XSS vectors like `<img onerror>`, `<svg onload>`, `javascript:` hrefs, and any unknown tags are stripped before HTML lands in the static site. Anyone who knows the editor password can still *overwrite* legitimate content with their own text, but they cannot inject script-equivalent payloads.
...
The sanitizer allowlist is part of this invariant. Both implementations strip the same editor-chrome attributes (`contenteditable`, `data-edit-key`, `data-start`/`data-end`/`data-is-only-node`/`data-is-last-node`), the same class (`.hg-editable`), the same `<div aria-hidden="true" class="pointer-events-none">` element, the same `<span style="background-color: ...">` declaration, and flatten the same nesting patterns. They then run the same XSS-prevention pass: allowlist tags `a, b, br, blockquote, em, i, li, ol, p, span, strong, u, ul`; allowed attribute `href` only on `<a>`, scheme-checked against `/^(?:https?:|mailto:|tel:|[/?#])|^[^:]*$/i`; drop containers `script, style, iframe, object, embed, link, meta, base, form, input, button, select, textarea, audio, video, svg, math`; unwrap everything else. Adding sanitization on one side without porting it produces drift where the browser shows clean overlays but the baked HTML still contains junk (or vice versa).
